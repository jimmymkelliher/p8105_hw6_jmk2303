...
We want to identify potential predictors of infant birth weight using data on mothers, fathers, and the infants themselves. Below, we pull in our dataset and factorize variables where appropriate.
...
As such, cleaning is a simple feat. That said, in the following sections, we condense and cull some of the predictors that might not be appropriate for our analysis. Even if all of the data are _clean_, that does not necessarily mean that the data are suitable for regression.
...
With our weather data in tow, we are ready to execute the bootstrap! We consider the simple regression
...
Next, let's try to estimate the distribution of the estimator for $S \equiv \log(\beta_0 \times \beta_1)$.

```{r distribution_logb0b1}
...
The 95% confidence interval around our estimate of $\log(\beta_0 \times \beta_1)$ is given by [`r round(quantile(pull(log_prod_estimate, log_prod), 0.05 / 2), 4)`, `r round(quantile(pull(log_prod_estimate, log_prod), 1 - 0.05 / 2), 4)`]. Once again, the distribution has a very slight left skew, but it largely seems to follow a normal distribution with mean `r round(mean(pull(log_prod_estimate, log_prod)), 4)` and standard deviation of `r round(sd(pull(log_prod_estimate, log_prod)), 4)`.